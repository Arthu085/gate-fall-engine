"""Configuração de treino da arma B0 (fusão pose+DINOv3), persistida como receita reproduzível por run.

Invariante experimental #1: A, B0 e as demais armas só podem diferir no vetor
de feature por timestep — todo campo da receita compartilhada abaixo é
copiado campo a campo de `BASELINE_A_CONFIG` (train/config.py). Só os campos
exclusivos de auditoria da fusão (dimensões, caminhos e hashes das duas
fontes de estatística) são específicos do B0.
"""
...
class B0TrainConfig:
    ...
    @staticmethod
    def from_dict(data: dict) -> "B0TrainConfig":
        return B0TrainConfig(**data)


B0_FUSION_CONFIG = B0TrainConfig(
    run_name="b0_fusion",
    arm="B0",
...
def save_config(config: B0TrainConfig, path: Path, force: bool) -> bool:
    ...
    print(f"{path}: configuração de treino B0 gravada (run_name={config.run_name})")
    return True


def load_config(path: Path) -> B0TrainConfig:
    with path.open("r", encoding="utf-8") as f:
        data = yaml.safe_load(f)
    return B0TrainConfig.from_dict(data)
