"""Selftest sintético da validação de artefatos de treino do C0
(`c0_artifacts.py`). Não toca em dados reais. Espelha `artifacts_selftest.py`
adaptado à arma C0 (fusão pose+SAM 3 V_t)."""
...
from gatefall.train.c0_artifacts import (
    REQUIRED_C0_TRAINING_ARTIFACTS,
    load_compatible_c0_checkpoint,
    validate_c0_training_metrics,
    validate_c0_training_run,
)
from gatefall.train.c0_config import C0_FUSION_CONFIG, C0TrainConfig, save_config
...
def _build_metrics_payload(config: C0TrainConfig) -> dict:
...
def check_per_class_rejected_when_matrix_inconsistent() -> bool:
    config = replace(C0_FUSION_CONFIG, epochs=1)
    ...
    raised = False
    try:
        validate_c0_training_metrics(data, config, config_path=None, checkpoint_path=None)
    except ValueError:
        raised = True

    return _check(
        "validate_c0_training_metrics rejeita per_class[0].fp que diverge de "
...
def check_malformed_per_class_rejected() -> bool:
    ...
    return _check(
        "validate_c0_training_metrics rejeita per_class[0] malformado (campo "
        "obrigatório 'tp' ausente)",
        raised,
    )


def _build_c0_model(config: C0TrainConfig) -> C0FusionClassifier:
...
def _write_valid_run(run_dir: Path, config: C0TrainConfig) -> None:
    run_dir.mkdir(parents=True, exist_ok=True)
    config_path = run_dir / "config.yaml"
    checkpoint_path = run_dir / "checkpoint.pt"
    save_config(config, config_path, force=True)
    model = _build_c0_model(config)
...
def check_seed_diff_accepted_by_checkpoint_compatibility() -> bool:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        config = replace(C0_FUSION_CONFIG, seed=1, epochs=1)
        ...
        try:
            ...
            accepted = False
        return _check(
            "validate_c0_training_run aceita checkpoint cuja config só "
...
def check_non_seed_diff_rejected_by_checkpoint_compatibility() -> bool:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        return _check(
            "validate_c0_training_run rejeita checkpoint cuja config diverge "
...
def check_required_artifacts_enforced() -> bool:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        raised = False
        try:
            validate_c0_training_run(run_dir, expected_config=config)
        except RuntimeError:
            raised = True
        return _check(
            f"validate_c0_training_run rejeita run com artefatos ausentes de "
            f"{REQUIRED_C0_TRAINING_ARTIFACTS}",
            raised,
        )


def check_load_compatible_c0_checkpoint_round_trip() -> bool:
    with tempfile.TemporaryDirectory() as tmp:
        run_dir = Path(tmp) / "run"
        config = replace(C0_FUSION_CONFIG, seed=1, epochs=1)
        _write_valid_run(run_dir, config)
        ...
        try:
            ...
            ok = isinstance(model, C0FusionClassifier)
        except ValueError:
            ok = False
        return _check(
            "load_compatible_c0_checkpoint carrega um checkpoint gravado com "
            "a mesma config",
            ok,
        )


def run_c0_artifacts_selftest() -> bool:
    checks = [
        ...
        check_load_compatible_c0_checkpoint_round_trip(),
    ]
    ok = all(checks)
    if not ok:
        print("\nc0 artifacts selftest FALHOU", file=sys.stderr)
    else:
        print("\nc0 artifacts selftest OK: todas as checagens passaram")
    return ok
