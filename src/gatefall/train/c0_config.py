"""Configuração de treino da arma C0 (fusão pose+SAM 3 V_t), persistida como receita reproduzível por run.

Invariante experimental #1: todo campo da receita compartilhada é copiado
campo a campo de `BASELINE_A_CONFIG` (train/config.py), como no B0. Só os
campos de auditoria da fusão (dimensões, estatísticas das duas fontes e
caminho, digest e proveniência do conjunto de `.h5` do SAM 3) são do C0.
"""
...
class C0TrainConfig:
    ...
    @staticmethod
    def from_dict(data: dict) -> "C0TrainConfig":
        return C0TrainConfig(**data)


C0_FUSION_CONFIG = C0TrainConfig(
    run_name="c0_fusion",
    arm="C0",
...
def save_config(config: C0TrainConfig, path: Path, force: bool) -> bool:
    ...
    print(f"{path}: configuração de treino C0 gravada (run_name={config.run_name})")
    return True


def load_config(path: Path) -> C0TrainConfig:
    with path.open("r", encoding="utf-8") as f:
        data = yaml.safe_load(f)
    return C0TrainConfig.from_dict(data)
