"""Selftest sintético do `B0FusionClassifier` (`b0_model.py`). Não toca em dados reais."""
...
def _build_model() -> B0FusionClassifier:
...
def run_b0_model_selftest() -> bool:
    checks = [
    ...
    ok = all(checks)
    if not ok:
        print("\nb0 model selftest FALHOU", file=sys.stderr)
    else:
        print("\nb0 model selftest OK: todas as checagens passaram")
    return ok
