"""Validação semântica dos artefatos persistidos de treino da arma C0."""
...
from gatefall.train.c0_config import C0TrainConfig, load_config
from gatefall.train.c0_model import C0FusionClassifier
from gatefall.train.metrics import RESTRICTED_CLASSES

REQUIRED_C0_TRAINING_ARTIFACTS = ("config.yaml", "metrics.json", "checkpoint.pt")
...
def validate_c0_training_run(
    ...
    present = [
        name for name in REQUIRED_C0_TRAINING_ARTIFACTS if (run_dir / name).is_file()
    ]
    if len(present) != len(REQUIRED_C0_TRAINING_ARTIFACTS):
        missing = [
            name for name in REQUIRED_C0_TRAINING_ARTIFACTS if name not in present
        ]
        raise RuntimeError(
    ...
    try:
        load_compatible_c0_checkpoint(run_dir / "checkpoint.pt", config)
