"""Dataset de janelas pose+DINOv3+qualidade (arma B1) e CLI de selftest."""
...
class GatedFusionWindowDataset:
    def __init__(
        self,
        frames: pd.DataFrame,
        split: str,
        ...
        self._quality_cache: dict[str, np.ndarray] = {}
    ...
    def _features_for_video(
        ...
        if video_id not in self._pose_cache:
            ...
            quality_array = self._quality_loader(video_id)
            ...
            if quality_array.shape[0] != expected_k:
                raise ValueError(
                    f"video_id={video_id!r}: fonte qualidade tem "
                    f"K={quality_array.shape[0]}, esperado K={expected_k} "
                    "(n_frames da tabela de frames)"
                )
            ...
            if quality_array.ndim != 2 or quality_array.shape[1] != QUALITY_CHANNELS:
                raise ValueError(
                    f"video_id={video_id!r}: fonte qualidade tem shape "
                    f"{quality_array.shape}, esperado "
                    f"({expected_k}, {QUALITY_CHANNELS}) — canal 0 = q_pose, "
                    "canal 1 = q_visual"
                )
            self._pose_cache[video_id] = pose_array
            self._visual_cache[video_id] = visual_array
            self._quality_cache[video_id] = quality_array
        return (
            self._pose_cache[video_id],
            self._visual_cache[video_id],
            self._quality_cache[video_id],
        )
    ...
    def __getitem__(
        ...
        label = int(row["label"])
        # Os mesmos índices de quadro cortam as três fontes: o alinhamento
        # temporal e a replicação de borda são estruturais, não verificados
        # a posteriori.
        frame_indices = window_frame_indices(k_end, n_frames)
        pose_array, visual_array, quality_array = self._features_for_video(video_id)
        pose_window = pose_array[frame_indices].astype(np.float32)
        visual_window = visual_array[frame_indices].astype(np.float32)
        quality_window = quality_array[frame_indices].astype(np.float32)
        return pose_window, visual_window, quality_window, label, (video_id, k_end)
...
def main() -> None:
    ...
    subparsers.add_parser(
        "selftest",
        help="Verifica o GatedFusionWindowDataset contra entradas sintéticas",
    )

    args = parser.parse_args()
    if args.command == "selftest":
        ok = run_gated_fusion_dataset_selftest()
        if not ok:
            raise SystemExit(1)
